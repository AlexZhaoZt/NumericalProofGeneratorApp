...
JNICALL
Java_com_bugting_www_numericalproofgenerator_result_strong(
        JNIEnv *env,
        jobject,
        jdouble key) {
    desired = double(key);
    ...
    strongProof();
    string* car = &arr[0];
    jar = (jobjectArray)env->NewObjectArray((jsize)arr.size(),
...
extern "C" JNIEXPORT jobjectArray
JNICALL
Java_com_bugting_www_numericalproofgenerator_result_weak(
        JNIEnv *env,
        jobject,
        jdouble key) {
    desired = double(key);
...
extern "C" JNIEXPORT jobjectArray
JNICALL
Java_com_bugting_www_numericalproofgenerator_result_sum(
        JNIEnv *env,
        jobject,
        jdouble key) {
    desired = double(key);
    jobjectArray jar;
    //Attempt Summation Proof.
    //尝试求和论证
    double sum = 0.0;
    string number = to_string(desired);
    int notZero = 0;
    ...
    for (size_t i = 0; i < number.size(); i++) {
        ...
        if (i == number.size() - 1) {
            sum_str += to_string(number.at(i) - '0');
        } else {
            sum_str += to_string(number.at(i) - '0') + "+";
        }
    }
...
void print(vector<int> seed) {
    ...
    arr.push_back(sum_str + std::to_string(desired) + "=" + translate(output));
}
